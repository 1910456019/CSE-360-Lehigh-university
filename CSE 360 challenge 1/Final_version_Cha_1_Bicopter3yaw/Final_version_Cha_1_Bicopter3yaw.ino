...
// if (PDterms.zEn) {
  ...
  // pd controller (to compute torque) --- Euler's rotation equation using desire angle for 0 and using tz as changed velocity
  
  // Speed Yaw
  //tz = PDterms.kpyaw * (atan2(sin(0 - yaw), cos(0 - yaw))) + PDterms.kdyaw * (tz - yawrate);

  // Angle Yaw
  
  tz = PDterms.kpyaw * (atan2(sin(tz - yaw), cos(tz - yaw))) + PDterms.kdyaw * (0 - yawrate);
  ...
  // if it match the certain heights, the value should be 0  
  if(fz = 0 or fz < 0){
    m1 = 0;
    m2 = 0;
  } else {
    m1 = sqrt(pow(fz1,2) + pow(fx1,2));
    m2 = sqrt(pow(fz2,2) + pow(fx2,2));
//      m1 = 1;
//      m2 = 1;
  }
...
//  }
  ...
  if(fx > 0 or fx < 0) {
    t1 = atan2(fz1, fx1 * -1) * (180/3.14);
    t2 = atan2(fz2, fx2) * (180/3.14);
    t1 = constrain(t1, 0, 180); // Angle 75 - 105, limiting yaw angle
    t2 = constrain(t2, 0, 180); // Angle 75 - 105, limiting yaw angle
    
  } else {
    t1 = atan2(fz1, fx1) * (180/3.14);
    t2 = atan2(fz2, fx2) * (180/3.14);
    t1 = constrain(t1, 60, 120); // Angle 75 - 105, limiting yaw angle
    t2 = constrain(t2, 60, 120); // Angle 75 - 105, limiting yaw angle
  }
//  t2 = atan2(fz2, fx2) * (180/3.14);
//  t1 = constrain(t1, 60, 120); // Angle 75 - 105, limiting yaw angle
//  t2 = constrain(t2, 60, 120); // Angle 75 - 105, limiting yaw angle
